  ...
  ui: function(params, cb) {
    ...
    if (FB._nativeInterface) {
        switch (params.method) {
            case 'auth.login':
                FB._nativeInterface.login(params, cb, function(e) {App.alert('Oops fail on login! Please try again later.');});
                break;
            ...
            case 'permissions.oauth':
                ...
                break;
            case 'auth.logout':
                FB._nativeInterface.logout(cb, function(e) {App.alert('Oops fail on logout! Please try again later.');});
                break;
            case 'auth.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {App.alert('Cordova Facebook Connect plugin fail on auth.status!');});
                break;
            case 'login.status':
                FB._nativeInterface.getLoginStatus(cb, function(e) {App.alert('Cordova Facebook Connect plugin fail on auth.status!');});
                break;
            case 'feed':
                FB._nativeInterface.dialog(params, cb, function(e) {App.alert('Cordova Facebook Connect plugin fail on auth.status!');});
                break;
            case 'apprequests':
                FB._nativeInterface.dialog(params, cb, function(e) {App.alert('Cordova Facebook Connect plugin fail on auth.status!');});
            break;
