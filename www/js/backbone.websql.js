...
_.extend(WebSQLStore.prototype,{
	...
	update: function (model, success, error) {
		...
		var self = this;
		var update = function() {
			self._executeSql("UPDATE `"+self.tableName+"` SET `value`=? WHERE(`id`=?);",[JSON.stringify(model.toJSON()), model.attributes[model.idAttribute]], success, error);
		};

		var res = this.find(model, function(trans, result) {

			if (result.rows.length) {
				update();
			} else {
				self.create(model, success, error);
			}
...
Backbone.sync = function (method, model, options) {
	...
	var store = model.store || (model.collection && model.collection.store), success, error;
	
	if (store == null) {
		Backbone.remoteSync(method, model, options);
		return;
	}
